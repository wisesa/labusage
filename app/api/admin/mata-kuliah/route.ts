...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const snapshot = await db.collection(getMataKuliahCollectionName()).get();

    const mata_kuliah = snapshot.docs
      .map((doc) => {
        const data = doc.data();
    ...
    return NextResponse.json({ mata_kuliah });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ref = await db.collection(getMataKuliahCollectionName()).add({
      kode,
      nama,
      active,
